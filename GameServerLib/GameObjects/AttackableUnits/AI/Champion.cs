...
namespace LeagueSandbox.GameServer.GameObjects.AttackableUnits.AI
{
    public class Champion : ObjAIBase
    {
        ...
        private float _botDecisionTimer;
        private float _botAbilityTimer;
        private int _botSkillBuildIndex;
        private static readonly byte[] BotLevelSixSkillBuild = { 0, 2, 1, 0, 0, 3 };
        ...
        private void UpdateBotAI(float diff)
        {
            LevelBotSkills();

            _botDecisionTimer -= diff;
            _botAbilityTimer -= diff;
            if (_botDecisionTimer > 0 || IsDead)
            {
                return;
            }

            _botDecisionTimer = 250.0f;

            var target = _game.ObjectManager.GetAllChampions()
                .Where(champion => champion.Team != Team
                    && !champion.IsDead
                    && champion.GetIsTargetableToTeam(Team))
                .OrderBy(champion => Vector2.DistanceSquared(Position, champion.Position))
                .FirstOrDefault();
            ...
            if (_botAbilityTimer <= 0 && Model.Equals("Garen", StringComparison.OrdinalIgnoreCase)
                && TryUseGarenAbility(target))
            {
                _botAbilityTimer = 3000.0f;
                return;
            }
            ...
            if (MoveOrder != OrderType.AttackTo && MoveOrder != OrderType.Hold)
            {
                UpdateMoveOrder(OrderType.AttackTo, true);
            }
        }

        private void LevelBotSkills()
        {
            while (_botSkillBuildIndex < BotLevelSixSkillBuild.Length && SkillPoints > 0)
            {
                var slot = BotLevelSixSkillBuild[_botSkillBuildIndex];
                if (LevelUpSpell(slot) == null)
                {
                    break;
                }
        ...
        private bool TryUseGarenAbility(Champion target)
        {
            var distanceSquared = Vector2.DistanceSquared(Position, target.Position);
            var targetHealthPercent = target.Stats.CurrentHealth / target.Stats.HealthPoints.Total;
            var healthPercent = Stats.CurrentHealth / Stats.HealthPoints.Total;

            if (targetHealthPercent <= 0.4f && TryCastBotSpell(3, target, distanceSquared))
            {
                return true;
            }

            if (healthPercent <= 0.65f && TryCastBotSpell(1, null, 0))
            {
                return true;
            }

            if (distanceSquared <= 900.0f * 900.0f && TryCastBotSpell(0, null, 0))
            {
                return true;
            }

            return distanceSquared <= 350.0f * 350.0f && TryCastBotSpell(2, null, 0);
        }

        private bool TryCastBotSpell(byte slot, AttackableUnit target, float distanceSquared)
        {
            if (!Spells.TryGetValue(slot, out var spell)
                || spell.CastInfo.SpellLevel == 0
            ...
            if (target != null && distanceSquared > spell.GetCurrentCastRange() * spell.GetCurrentCastRange())
            {
                return false;
            }

            var castPosition = target?.Position ?? Position;
            return spell.Cast(castPosition, castPosition, target);
        }
